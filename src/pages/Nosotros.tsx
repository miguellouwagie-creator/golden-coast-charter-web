...
const Nosotros = () => {
  ...
  const values = [
    ...
    { icon: Star, title: t("about.value3.title"), description: t("about.value3.desc") },
    { icon: Users, title: t("about.value4.title"), description: t("about.value4.desc") },
  ];

  const certifications = [
    t("about.cert1"), t("about.cert2"), t("about.cert3"),
    t("about.cert4"), t("about.cert5"),
  ];

  const testimonials = [
    { name: t("about.test1.name"), location: t("about.test1.location"), rating: 5, text: t("about.test1.text") },
    { name: t("about.test2.name"), location: t("about.test2.location"), rating: 5, text: t("about.test2.text") },
    { name: t("about.test3.name"), location: t("about.test3.location"), rating: 5, text: t("about.test3.text") },
  ];
    ...
    <div className="min-h-screen bg-background">
              ...
              <div className="relative grid grid-cols-1 lg:grid-cols-5 gap-0 min-h-[700px]">
                ...
                <div className="lg:col-span-3 p-8 md:p-12 lg:p-16 flex flex-col justify-center bg-white">
                  <div className="flex items-center justify-start mb-8">
                    <Anchor className="h-16 w-16 text-gold" aria-hidden="true" />
                  </div>
                  ...
                  <div className="space-y-6 text-muted-foreground leading-relaxed text-lg">
                    ...
                    <p className="font-semibold text-primary text-xl">{t("about.story.p4")}</p>
                    <p>{t("about.story.p5")}</p>
                    <p>{t("about.story.p6")}</p>
                    <p className="font-semibold text-primary text-xl">{t("about.story.p7")}</p>
                  </div>
                </div>

                {/* Captain image — 2 cols */}
                <div
                ...
                >
                  ...
                  <div
                    ...
                    style={{ background: 'linear-gradient(to right, rgba(0,0,0,0.10) 0%, rgba(0,0,0,0) 25%)' }}
                  />
                </div>
              </div>
            </CardContent>
      ...
      <section className="py-20 px-4 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0f2557 0%, #1a3a5c 50%, #0a192f 100%)' }}>
        ...
        <div className="container mx-auto relative z-10">
          <h2
            ...
            style={{ textShadow: '0 0 30px rgba(255, 215, 0, 0.3), 0 0 60px rgba(255, 215, 0, 0.1)' }}
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {values.map((value, index) => (
              <Card
                ...
                style={{ boxShadow: '0 10px 40px rgba(0,0,0,0.3), 0 0 1px rgba(255,215,0,0.2)' }}
              >
                <CardContent className="p-8 relative">
                  ...
                  <div
                    ...
                    style={{ background: 'linear-gradient(135deg, rgba(255,215,0,0.1) 0%, rgba(255,165,0,0.05) 100%)', boxShadow: '0 4px 20px rgba(255,215,0,0.2), inset 0 0 20px rgba(255,215,0,0.1)' }}
                  >
                    <value.icon className="h-8 w-8 text-gold" aria-hidden="true" />
                  </div>
                  <h3 className="font-heading text-xl font-semibold text-primary mb-4">{value.title}</h3>
      ...
      <section className="py-20 px-4">
        <div className="container mx-auto max-w-4xl">
          <div className="text-center mb-12">
            <Award className="h-16 w-16 text-gold mx-auto mb-6" aria-hidden="true" />
            <h2 className="font-heading text-4xl font-bold text-primary mb-4">{t("about.certs.title")}</h2>
            <p className="text-xl text-muted-foreground">{t("about.certs.subtitle")}</p>
          </div>
              ...
              <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {certifications.map((cert, index) => (
                  <li key={index} className="flex items-center space-x-3">
                    <Shield className="h-6 w-6 text-gold flex-shrink-0" aria-hidden="true" />
                    <span className="text-lg">{cert}</span>
                  </li>
                ))}
              </ul>
              <div className="mt-8 pt-8 border-t border-primary-foreground/20 text-center">
                <p className="text-primary-foreground/90">{t("about.certs.footer")}</p>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      <section className="py-20 px-4 bg-secondary">
        <div className="container mx-auto">
          <div className="text-center mb-16">
            <h2 className="font-heading text-4xl font-bold text-primary mb-4">{t("about.testimonials.title")}</h2>
            <p className="text-xl text-muted-foreground">{t("about.testimonials.subtitle")}</p>
          </div>
      ...
      <section className="py-20 px-4">
        <div className="container mx-auto max-w-4xl text-center">
          <h2 className="font-heading text-4xl font-bold text-primary mb-6">{t("about.cta.title")}</h2>
          <p className="text-xl text-muted-foreground mb-10">{t("about.cta.subtitle")}</p>
